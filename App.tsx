...
const App: React.FC = () => {
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setMessage("Analysiere Bild und erkenne Objekte...");
  ...
  useEffect(() => {
    if (speechError) {
      if (speechError === 'no-speech') {
        setMessage('Ich konnte dich nicht hören. Bitte versuche es noch einmal und sprich deutlich.');
      } else if (speechError === 'not-allowed' || speechError === 'service-not-allowed') {
        setMessage('Der Mikrofonzugriff wurde verweigert. Bitte erlaube den Zugriff in deinen Browsereinstellungen.');
        setGameState(GameState.ERROR);
      } else {
        setMessage(`Ein Spracherkennungsfehler ist aufgetreten: ${speechError}.`);
        setGameState(GameState.ERROR);
      }
    }
  }, [speechError]);
